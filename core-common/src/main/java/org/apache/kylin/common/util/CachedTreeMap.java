...
public class CachedTreeMap<K extends ComparableWritable, V extends Writable> extends TreeMap<K, V> implements Writable {
    ...
    private CachedTreeMap(int maxCount, Class<K> keyClazz, Class<V> valueClazz, String baseDir, boolean persistent, boolean immutable) {
        ...
        this.persistent = persistent;
        this.immutable = immutable;
        this.valueCache = CacheBuilder.newBuilder().removalListener(new RemovalListener<K, V>() {
            @Override
            public void onRemoval(RemovalNotification<K, V> notification) {
                logger.debug(String.format("Evict cache key %s(%d) with value %s caused by %s, size %d/%d ",
        ...
        }).maximumSize(maxCount).build(new CacheLoader<K, V>() {
            @Override
            public V load(K key) throws Exception {
                V value = readValue(key);
                logger.debug(String.format("Load cache by key %s(%d) with value %s", key, key.hashCode(), value));
                return value;
            }
        });
    }
    ...
    // TODO maybe check the exist file and just skip if no data updated
